...
/// sets <c>IsEmailOtpEnabled = true</c> on the user, and emits the matching audit event. Locks
/// the code (invalidates it via <c>GcRecord</c>) after 5 failed attempts — same criterion as
/// <c>ConfirmPhoneVerificationCommandHandler</c>.
/// </summary>
public sealed class EnableEmailOtpCommandHandler(
    UserManager<ApplicationUser> userManager,
    ICurrentUserService currentUserService,
    IEmailOtpEnableCodeRepository emailOtpCodeRepository,
    ISecurityEventLogger securityEventLogger)
    : IRequestHandler<EnableEmailOtpCommand, Response<bool>>
{
    ...
    public async Task<Response<bool>> Handle(EnableEmailOtpCommand request, CancellationToken cancellationToken)
    {
        ...
        if (stored is null)
        {
            return Response<bool>.Error("EMAIL_OTP_NOT_REQUESTED", ["Request a code via send-code before enabling email OTP."]);
        }
        ...
        if (!used)
        {
            // Concurrent enable raced and won — treat as expired so we don't double-flip the flag.
            return Response<bool>.Error("EMAIL_OTP_CODE_EXPIRED", ["The verification code has expired. Request a new one."]);
        }
