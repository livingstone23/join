...
/// <summary>
/// Handles soft delete operations for customer contacts using Entity Framework Core.
/// </summary>
...
public sealed class DeleteCustomerContactCommandHandler(
    IUnitOfWork unitOfWork,
    ICurrentUserService currentUserService) : IRequestHandler<DeleteCustomerContactCommand, Response<bool>>
{
    private readonly IUnitOfWork _unitOfWork = unitOfWork;

    /// <summary>
    /// Marks the customer contact as logically deleted for the current tenant.
    /// </summary>
    ...
    public async Task<Response<bool>> Handle(DeleteCustomerContactCommand request, CancellationToken cancellationToken)
    {
        ...
        var repository = _unitOfWork.GetRepository<CustomerContact>();
        ...
        if (entity is null)
        {
            return Response<bool>.Error(
                ...
                ["No active contact was found for the current tenant."]);
        }
        ...
        if (affected <= 0)
        {
            return Response<bool>.Error(
                "DELETE_FAILED",
                ["No records were affected while deleting the customer contact."]);
        }
