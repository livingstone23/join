...
/// <summary>
/// Handles the update of an existing Person aggregate, including collection synchronization.
/// </summary>
    ...
    ICurrentUserService currentUserService) : IRequestHandler<UpdatePersonCommand, Response<Guid>>
{
    ...
    public async Task<Response<Guid>> Handle(UpdatePersonCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            return Response<Guid>.Error("CUSTOMER_NOT_FOUND", ["Person not found for the current company."]);
        }
        ...
        var duplicatedIdentification = await _unitOfWork.Persons.ExistsByCompanyAndIdentificationExceptIdAsync(
        ...
        await SyncContacts(request, customerEntity, currentUserService.CompanyId, collectionSyncErrors, cancellationToken);
    ...
    private async Task SyncAddresses(
