    ...
    : IRequestHandler<GetPersonsPagedQuery, Response<PagedResult<PersonListItemDto>>>
{
    ...
    public async Task<Response<PagedResult<PersonListItemDto>>> Handle(GetPersonsPagedQuery request, CancellationToken cancellationToken)
    {
        var response = new Response<PagedResult<PersonListItemDto>>();
        ...
        var items = (await multi.ReadAsync<PersonListItemDto>()).AsList();
        var totalCount = await multi.ReadSingleAsync<int>();

        response.Data = new PagedResult<PersonListItemDto>
        {
            Items = items,
        ...
        response.Message = "Persons retrieved successfully.";
