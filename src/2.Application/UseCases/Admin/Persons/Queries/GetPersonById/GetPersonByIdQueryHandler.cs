    ...
    : IRequestHandler<GetPersonByIdQuery, Response<PersonDto>>
{
    ...
    public async Task<Response<PersonDto>> Handle(GetPersonByIdQuery request, CancellationToken cancellationToken)
    {
        ...
        {
            return Response<PersonDto>.Error(
                "COMPANY_REQUIRED",
        ...
        const string sql = """
            -- Query 1: Fetch Person with Identification Type Name
            SELECT 
            ...
            -- Query 2: Fetch Person Addresses with Country, Province, etc.
            ...
            -- Query 3: Fetch Person Contacts
            SELECT 
                co.Id, co.ContactType, co.ContactValue, co.IsPrimary, co.Comments, co.Created
            FROM Admin.PersonContacts co
            WHERE co.PersonId = @Id AND co.CompanyId = @TenantId AND co.GcRecord = 0;
            """;

        // 3. Execute the multiple queries via Dapper
        using var multi = await connection.QueryMultipleAsync(sql, new 
        { 
            Id = request.PersonId, // Ensure this matches the property in GetPersonByIdQuery
            TenantId = currentUserService.CompanyId 
        });

        // 4. Map the first result set directly to PersonDto
        var customerDto = await multi.ReadFirstOrDefaultAsync<PersonDto>();
        ...
        // 6. Map Addresses dynamically matching the PersonAddressDto properties
        ...
        // 7. Map Contacts dynamically matching the PersonContactDto properties
        var contacts = contactsRaw.Any()
            ? contactsRaw.Select(c => new PersonContactDto
                {
                    Id = c.Id,
