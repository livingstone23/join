...
public sealed class UpdateCustomerAddressCommandHandler(
    ...
    ICurrentUserService currentUserService) : IRequestHandler<UpdateCustomerAddressCommand, Response<Guid>>
{
    ...
    public async Task<Response<Guid>> Handle(UpdateCustomerAddressCommand request, CancellationToken cancellationToken)
    {
        ...
        if (entity is null)
        {
            throw new NotFoundException(
                ...
                request.Id,
                "Customer address not found for the current tenant.");
        }
        ...
        if (entity.CustomerId != request.CustomerId)
        {
            throw new NotFoundException(
                nameof(CustomerAddress),
                request.Id,
                "Customer address not found for the requested customer.");
        }
