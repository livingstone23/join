...
/// <summary>
/// Handles identification type update commands using the transactional write stack.
/// </summary>
/// <param name="unitOfWork">Unit of work used for transactional persistence.</param>
public sealed class UpdateIdentificationTypeCommandHandler(IUnitOfWork unitOfWork)
    : IRequestHandler<UpdateIdentificationTypeCommand, Response<IdentificationTypeDto>>
{
    private readonly IUnitOfWork _unitOfWork = unitOfWork;

    /// <summary>
    /// Updates an existing identification type after validating uniqueness.
    /// </summary>
    ...
    public async Task<Response<IdentificationTypeDto>> Handle(UpdateIdentificationTypeCommand request, CancellationToken cancellationToken)
    {
        ...
        var nameInUse = existingIdentificationTypes.Any(identificationType =>
            identificationType.Id != request.Id
        ...
        entity.ValidationPattern = normalizedValidationPattern;
        ...
        if (result <= 0)
        {
            return Response<IdentificationTypeDto>.Error(
                "UPDATE_FAILED",
                ["No records were affected while updating the identification type."]);
        }
