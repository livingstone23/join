...
/// <summary>
/// Handles ticket complexity update commands.
/// </summary>
/// <param name="unitOfWork">Unit of work used for transactional persistence.</param>
public sealed class UpdateTicketComplexityCommandHandler(IUnitOfWork unitOfWork)
    : IRequestHandler<UpdateTicketComplexityCommand, Response<TicketComplexityDto>>
{
    private readonly IUnitOfWork _unitOfWork = unitOfWork;

    /// <summary>
    /// Updates a ticket complexity catalog item.
    /// </summary>
    public async Task<Response<TicketComplexityDto>> Handle(UpdateTicketComplexityCommand request, CancellationToken cancellationToken)
    {
        ...
        entity.ResolutionTimeUnits = request.ResolutionTimeUnits;
        ...
        if (result <= 0)
        {
            return Response<TicketComplexityDto>.Error("UPDATE_FAILED", ["No records were affected while updating the ticket complexity."]);
        }

        return new Response<TicketComplexityDto>
        {
            IsSuccess = true,
            Message = "Ticket complexity updated successfully.",
