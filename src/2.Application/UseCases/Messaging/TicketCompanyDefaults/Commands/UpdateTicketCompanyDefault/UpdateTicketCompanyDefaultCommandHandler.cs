...
/// <summary>
/// Handles tenant ticket default configuration updates.
/// </summary>
public sealed class UpdateTicketCompanyDefaultCommandHandler(
    IUnitOfWork unitOfWork,
    ICurrentUserService currentUserService,
    ITicketCompanyDefaultMapper mapper)
    : IRequestHandler<UpdateTicketCompanyDefaultCommand, Response<TicketCompanyDefaultDto>>
{
    private readonly IUnitOfWork _unitOfWork = unitOfWork;
    private readonly ITicketCompanyDefaultMapper _mapper = mapper;

    /// <summary>
    /// Updates the tenant configuration if it belongs to the active company.
    /// </summary>
    public async Task<Response<TicketCompanyDefaultDto>> Handle(UpdateTicketCompanyDefaultCommand request, CancellationToken cancellationToken)
    {
        if (currentUserService.CompanyId == Guid.Empty)
        {
            return Response<TicketCompanyDefaultDto>.Error("COMPANY_REQUIRED", ["The X-Company-Id header is required."]);
        }
        ...
        if (result <= 0)
        {
            return Response<TicketCompanyDefaultDto>.Error("UPDATE_FAILED", ["No records were affected while updating the configuration."]);
        }

        return new Response<TicketCompanyDefaultDto>
        {
            IsSuccess = true,
            Message = "Ticket company default configuration updated successfully.",
            Data = BuildDto(entity, status?.Name, complexity?.Name, timeUnit?.Name, area?.Name, project?.Name, channel?.Name)
        };
    }

    private async Task<(Response<TicketCompanyDefaultDto>? Error, TicketStatus? Status, TicketComplexity? Complexity, TimeUnit? TimeUnit, Area? Area, Project? Project, CommunicationChannel? Channel)> ValidateReferencesAsync(UpdateTicketCompanyDefaultCommand request)
    {
