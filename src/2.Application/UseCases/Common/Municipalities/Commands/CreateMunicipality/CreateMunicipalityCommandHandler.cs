...
/// <summary>
/// Handles municipality creation commands.
/// </summary>
/// <param name="unitOfWork">Unit of work used for transactional persistence.</param>
public sealed class CreateMunicipalityCommandHandler(IUnitOfWork unitOfWork)
    : IRequestHandler<CreateMunicipalityCommand, Response<MunicipalityDto>>
{
    private readonly IUnitOfWork _unitOfWork = unitOfWork;

    /// <summary>
    /// Creates a municipality catalog item.
    /// </summary>
    /// <param name="request">The creation payload.</param>
    /// <param name="cancellationToken">The operation cancellation token.</param>
    /// <returns>A standardized response containing the created municipality.</returns>
    public async Task<Response<MunicipalityDto>> Handle(CreateMunicipalityCommand request, CancellationToken cancellationToken)
    {
        ...
        await municipalityRepository.InsertAsync(entity);
        ...
        if (result <= 0)
        {
            return Response<MunicipalityDto>.Error(
                "CREATE_FAILED",
                ["No records were affected while creating the municipality."]);
        }
