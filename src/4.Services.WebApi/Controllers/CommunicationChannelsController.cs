...
/// <summary>
/// API controller for managing communication channel catalog entities.
/// </summary>
[ApiController]
...
public class CommunicationChannelsController(IMediator mediator) : ControllerBase
{
    ...
    {
        var response = await _mediator.Send(new GetCommunicationChannelByIdQuery(id));
    ...
    {
        var response = await _mediator.Send(new GetCommunicationChannelsPagedQuery(pageNumber, pageSize, searchTerm));
        if (!response.IsSuccess)
        {
    ...
    public async Task<IActionResult> Create([FromBody] CreateCommunicationChannelCommand command)
    {
    ...
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [ProducesResponseType(StatusCodes.Status409Conflict)]
    public async Task<IActionResult> Update(Guid id, [FromBody] UpdateCommunicationChannelCommand command)
    {
    ...
    {
        var response = await _mediator.Send(new DeleteCommunicationChannelCommand(id));
        if (!response.IsSuccess)
        {
            if (response.Message == "COMMUNICATIONCHANNEL_NOT_FOUND")
            {
