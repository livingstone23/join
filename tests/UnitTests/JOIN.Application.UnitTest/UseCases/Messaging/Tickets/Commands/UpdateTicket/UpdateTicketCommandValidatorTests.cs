...
/// <summary>
/// Contains unit tests for <see cref="UpdateTicketCommandValidator"/>.
/// Each test exercises a single validation rule in isolation.
/// </summary>
public sealed class UpdateTicketCommandValidatorTests
{
    private readonly UpdateTicketCommandValidator _validator = new();

    // ──────────────────────────────────────────────
    //  Id rule (unique to Update)
    // ──────────────────────────────────────────────

    /// <summary>
    /// Verifies that an empty Id triggers the required error.
    /// </summary>
    [Fact]
    public void Validate_WhenIdIsEmpty_ShouldReturnIdRequiredError()
    {
        ...
        result.Errors.Should().ContainSingle(x =>
            x.PropertyName == nameof(UpdateTicketCommand.Id) &&
    ...
    public void Validate_WhenNameIsEmpty_ShouldReturnNameRequiredError()
    {
        ...
        result.Errors.Should().ContainSingle(x =>
            x.PropertyName == nameof(UpdateTicketCommand.Name) &&
    ...
    public void Validate_WhenDescriptionIsEmpty_ShouldReturnDescriptionRequiredError()
    {
        ...
        result.Errors.Should().ContainSingle(x =>
            x.PropertyName == nameof(UpdateTicketCommand.Description) &&
    ...
    public void Validate_WhenEstimatedTimeIsNegative_ShouldReturnRangeError()
    {
        ...
        result.Errors.Should().ContainSingle(x =>
            x.PropertyName == nameof(UpdateTicketCommand.EstimatedTime) &&
    ...
    public void Validate_WhenConsumedTimeIsNegative_ShouldReturnRangeError()
    {
        ...
        result.Errors.Should().ContainSingle(x =>
            x.PropertyName == nameof(UpdateTicketCommand.ConsumedTime) &&
    ...
    public void Validate_WhenEffortPointsIsNegative_ShouldReturnNegativeEffortError()
    {
        ...
        result.Errors.Should().ContainSingle(x =>
            x.PropertyName == nameof(UpdateTicketCommand.EffortPoints) &&
    ...
    /// </summary>
    [Theory]
    [InlineData(nameof(UpdateTicketCommand.TicketStatusId), "Ticket status is required.")]
    [InlineData(nameof(UpdateTicketCommand.TicketComplexityId), "Ticket complexity is required.")]
    [InlineData(nameof(UpdateTicketCommand.TimeUnitId), "Time unit is required.")]
    [InlineData(nameof(UpdateTicketCommand.ChannelId), "Channel is required.")]
    ...
    // ──────────────────────────────────────────────
    //  Self-reference rule (unique to Update)
    // ──────────────────────────────────────────────

    /// <summary>
    /// Verifies that setting PrecedentTicketId equal to Id triggers the self-reference error.
    /// The rule is object-level, so the PropertyName is an empty string.
    /// </summary>
    [Fact]
    public void Validate_WhenPrecedentTicketIdEqualsSelf_ShouldReturnSelfReferenceError()
    {
        var id = Guid.NewGuid();
        var command = CreateValidCommand() with { Id = id, PrecedentTicketId = id };

        var result = _validator.Validate(command);

        result.Errors.Should().ContainSingle(x =>
            x.ErrorMessage == "A ticket cannot reference itself as precedent.");
    }
    ...
    /// </summary>
    private static UpdateTicketCommand CreateValidCommand() => new()
    {
        Id = Guid.NewGuid(),
    ...
    /// </summary>
    private static UpdateTicketCommand SetRequiredGuidProperty(UpdateTicketCommand command, string propertyName, Guid value) =>
        propertyName switch
        {
            nameof(UpdateTicketCommand.TicketStatusId) => command with { TicketStatusId = value },
            nameof(UpdateTicketCommand.TicketComplexityId) => command with { TicketComplexityId = value },
            nameof(UpdateTicketCommand.TimeUnitId) => command with { TimeUnitId = value },
            nameof(UpdateTicketCommand.ChannelId) => command with { ChannelId = value },
