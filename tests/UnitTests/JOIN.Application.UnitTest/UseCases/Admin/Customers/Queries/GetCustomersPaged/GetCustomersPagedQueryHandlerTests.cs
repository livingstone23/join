...
public sealed class GetCustomersPagedQueryHandlerTests
{
    ...
    {
        // Arrange
        var context = new GetCustomersPagedQueryTestContext(Guid.Empty, useNpgsqlConnection: false);
    ...
    /// </summary>
    [Fact]
    public async Task Handle_WhenFiltersAreProvided_ShouldApplyTenantFiltersAndReturnPagedCustomers()
    {
        // Arrange
        var companyId = _fixture.Create<Guid>();
        var identificationTypeId = _fixture.Create<Guid>();
        var context = new GetCustomersPagedQueryTestContext(companyId, useNpgsqlConnection: true);

        context.Connection.SetResults(
            CreateCustomerItemsResultSet(companyId, identificationTypeId),
    ...
    /// </summary>
    [Fact]
    public async Task Handle_WhenNoCustomersMatch_ShouldReturnEmptyPagedResult()
    {
        // Arrange
        var companyId = _fixture.Create<Guid>();
        var context = new GetCustomersPagedQueryTestContext(companyId, useNpgsqlConnection: false);
        ...
        var query = new GetCustomersPagedQuery(PageNumber: -5, PageSize: 0, FirstName: "missing");
        var handler = context.CreateHandler();
    ...
    /// </summary>
    private static FakeResultSet CreateCustomerItemsResultSet(Guid companyId, Guid identificationTypeId)
    {
    ...
    /// </summary>
    private sealed class GetCustomersPagedQueryTestContext
    {
        public GetCustomersPagedQueryTestContext(Guid companyId, bool useNpgsqlConnection)
        {
        ...
        {
            return new GetCustomersPagedQueryHandler(
