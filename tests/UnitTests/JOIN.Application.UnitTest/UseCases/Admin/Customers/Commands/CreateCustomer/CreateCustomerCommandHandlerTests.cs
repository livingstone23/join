...
public sealed class CreateCustomerCommandHandlerTests
{
    ...
    /// </summary>
    [Fact]
    public async Task Handle_WhenRequestIsValid_ShouldCreateCustomerAndPopulateChildTenantData()
    {
        // Arrange
        var companyId = _fixture.Create<Guid>();
        var request = CreateValidCommand(includeAddresses: true, includeContacts: true);
        var customerEntity = CreateMappedCustomer(request);
        ...
        customerEntity.Addresses.Should().OnlyContain(x =>
            x.CompanyId == companyId && x.CustomerId == customerEntity.Id);
        customerEntity.Contacts.Should().OnlyContain(x =>
            x.CompanyId == companyId && x.CustomerId == customerEntity.Id);

        context.MapperMock.Verify(x => x.ToEntity(request), Times.Once);
        context.CustomersRepositoryMock.Verify(x => x.InsertAsync(customerEntity), Times.Once);
    ...
    public async Task Handle_WhenCompanyIdIsEmpty_ShouldReturnCompanyRequiredError()
    {
        ...
        context.CustomersRepositoryMock.Verify(x => x.InsertAsync(It.IsAny<Customer>()), Times.Never);
    ...
    public async Task Handle_WhenCompanyDoesNotExist_ShouldReturnInvalidCompanyError()
    {
        ...
        context.MapperMock.Verify(x => x.ToEntity(It.IsAny<CreateCustomerCommand>()), Times.Never);
        context.CustomersRepositoryMock.Verify(x => x.InsertAsync(It.IsAny<Customer>()), Times.Never);
        context.UnitOfWorkMock.Verify(x => x.SaveAsync(It.IsAny<CancellationToken>()), Times.Never);
    }
    ...
    public async Task Handle_WhenAddressReferencesAreInvalid_ShouldReturnAggregatedAddressErrors()
    {
        ...
        context.CustomersRepositoryMock.Verify(
    ...
    /// </summary>
    [Fact]
    public async Task Handle_WhenCustomerAlreadyExists_ShouldReturnCustomerAlreadyExistsError()
    {
        // Arrange
        var companyId = _fixture.Create<Guid>();
        var context = CreateContext(companyId);
        ...
        bool invalidContactType = false)
    {
        var addresses = includeAddresses
            ...
            : Array.Empty<CreateCustomerCommand.CreateCustomerAddressDto>();
        ...
        var contacts = includeContacts
            ...
            : Array.Empty<CreateCustomerCommand.CreateCustomerContactDto>();

        return _fixture.Build<CreateCustomerCommand>()
    ...
    /// </summary>
    private static Customer CreateMappedCustomer(CreateCustomerCommand request)
    {
        ...
        return new Customer
        {
            ...
            Addresses = request.Addresses?.Select(address => new CustomerAddress
            {
    ...
    /// </summary>
    private static void SetupValidAddressReferences(CreateCustomerTestContext context, CreateCustomerCommand request)
    {
    ...
    /// </summary>
    private static CreateCustomerTestContext CreateContext(Guid companyId)
    {
        return new CreateCustomerTestContext(companyId);
    }
    ...
    /// </summary>
    private sealed class CreateCustomerTestContext
    {
        public CreateCustomerTestContext(Guid companyId)
        {
